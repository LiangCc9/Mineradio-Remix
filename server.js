...
const PLAYLIST_TRACK_CACHE_TTL_MS = 5 * 60 * 1000;
const PLAYLIST_TRACK_PAGE_LIMIT = 200;
const PLAYLIST_TRACK_SAFE_LIMIT = 5000;
const PLAYLIST_TRACK_CACHE_MAX = 24;
...
function saveCookie(c) {
  ...
  if (nextCookie !== userCookie) playlistTrackCache.clear();
...
function playlistTrackIdValue(item) {
  if (item == null) return '';
  if (typeof item === 'object') return item.id == null ? '' : String(item.id);
  return String(item);
}

function playlistTrackCacheSongKey(song) {
  song = song || {};
  const id = song.id || song.mid || song.songmid || song.programId || song.mainTrackId || '';
  if (id || id === 0) return String(song.provider || song.source || 'netease') + ':id:' + String(id);
  return String(song.provider || song.source || 'netease') + ':meta:' +
    [song.name || '', song.artist || '', song.album || ''].join('|').toLowerCase();
}

function mergePlaylistTrackCacheSongs(base, incoming) {
  const out = [];
  const seen = new Set();
  (base || []).concat(incoming || []).forEach(song => {
    if (!song) return;
    const key = playlistTrackCacheSongKey(song);
...
function freshPlaylistTrackCacheEntry(id) {
  const key = String(id);
  const entry = playlistTrackCache.get(key);
  if (!entry) return null;
  if (Date.now() - Number(entry.savedAt || 0) >= PLAYLIST_TRACK_CACHE_TTL_MS) {
    playlistTrackCache.delete(key);
    return null;
  }
  return entry;
}

function storePlaylistTrackCacheEntry(id, entry) {
  const key = String(id);
  playlistTrackCache.delete(key);
  playlistTrackCache.set(key, entry);
  while (playlistTrackCache.size > PLAYLIST_TRACK_CACHE_MAX) {
    playlistTrackCache.delete(playlistTrackCache.keys().next().value);
  }
  return entry;
}

function playlistMetaFromDetail(id, playlist, expectedTotal) {
  playlist = playlist || {};
  const trackIdSource = Array.isArray(playlist.trackIds) && playlist.trackIds.length
    ? playlist.trackIds
    : (Array.isArray(playlist.tracks) ? playlist.tracks : []);
  const trackIds = trackIdSource.map(playlistTrackIdValue).filter(Boolean);
  ...
  return {
    playlist: {
      ...
      trackCount: declaredTotal
    },
    total: declaredTotal,
    trackIds: trackIds.slice(0, PLAYLIST_TRACK_SAFE_LIMIT)
  };
}

function orderRawSongsByIds(rawSongs, ids) {
  const byId = new Map();
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/song/like') {
    try {
      ...
      if (code === 200) playlistTrackCache.clear();
  ...
  if (pn === '/api/playlist/add-song') {
    try {
      ...
      playlistTrackCache.delete(String(pid));
  ...
  if (pn === '/api/playlist/tracks') {
    try {
      ...
      const requestedLimit = Number.parseInt(url.searchParams.get('limit') || String(PLAYLIST_TRACK_PAGE_LIMIT), 10);
      const requestedOffset = Number.parseInt(url.searchParams.get('offset') || '0', 10);
      const requestedTotal = Number.parseInt(url.searchParams.get('total') || '0', 10);
      const offset = Number.isFinite(requestedOffset) ? Math.max(0, requestedOffset) : 0;
      const expectedTotal = Number.isFinite(requestedTotal) ? Math.max(0, Math.min(1000000, requestedTotal)) : 0;
      if (offset >= PLAYLIST_TRACK_SAFE_LIMIT) {
        sendJSON(res, {
          playlist: { id, name: '', cover: '', trackCount: expectedTotal },
          tracks: [], total: expectedTotal, nextOffset: PLAYLIST_TRACK_SAFE_LIMIT,
          more: false, complete: false, truncated: true, cached: false,
        });
        return;
      }
      const limit = Math.max(1, Math.min(
        PLAYLIST_TRACK_PAGE_LIMIT,
        PLAYLIST_TRACK_SAFE_LIMIT - offset,
        Number.isFinite(requestedLimit) ? requestedLimit : PLAYLIST_TRACK_PAGE_LIMIT
      ));
      let cached = freshPlaylistTrackCacheEntry(id);
      if (cached && cached.complete === true) {
        const sliced = cached.tracks.slice(offset, offset + limit);
        const nextOffset = offset + sliced.length;
        const pageComplete = nextOffset >= cached.tracks.length;
        sendJSON(res, {
          playlist: cached.playlist,
          tracks: sliced,
          total: Math.max(Number(cached.total) || 0, cached.tracks.length),
          nextOffset,
          more: !pageComplete,
      ...
      let playlistMeta = cached && cached.playlist || { id, name: '', cover: '', trackCount: expectedTotal };
      let total = Math.max(expectedTotal, Number(cached && cached.total) || 0);
      let trackIds = cached && Array.isArray(cached.trackIds) ? cached.trackIds : [];
      let metadataResolved = !!(cached && cached.metadataResolved);
      let rawTracks = [];
      let rawPageCount = 0;

      // Resolve the ID list once, then request only one small song-detail page.
      // This keeps the first paint fast and avoids re-downloading a 600+ track
      // playlist for every background page.
      if (!metadataResolved && typeof playlist_detail === 'function') {
        try {
          const detail = await playlist_detail({ id, s: 0, cookie: userCookie, timestamp: Date.now() });
          const pl = (detail.body && detail.body.playlist) || {};
          const normalized = playlistMetaFromDetail(id, pl, expectedTotal);
          playlistMeta = normalized.playlist;
          total = normalized.total;
          trackIds = normalized.trackIds;
          metadataResolved = trackIds.length > 0 || total === 0;
        } catch (err) {
          console.warn('[PlaylistTracks] playlist_detail metadata failed, using paged fallback:', err.message);
        }
      }

      if (metadataResolved) {
        const pageIds = trackIds.slice(offset, offset + limit);
        rawPageCount = pageIds.length;
        if (pageIds.length) {
          try {
            const detail = await song_detail({ ids: pageIds.join(','), cookie: userCookie, timestamp: Date.now() });
            rawTracks = orderRawSongsByIds((detail.body && detail.body.songs) || [], pageIds);
          } catch (err) {
            if (typeof playlist_track_all !== 'function') throw err;
            console.warn('[PlaylistTracks] song_detail page failed, using playlist_track_all:', err.message);
            const fallback = await playlist_track_all({ id, limit, offset, cookie: userCookie, timestamp: Date.now() });
            const body = fallback.body || {};
            rawTracks = body.songs || body.tracks || [];
          }
        }
      } else {
        if (typeof playlist_track_all !== 'function') throw new Error('PLAYLIST_TRACK_API_UNAVAILABLE');
        const fallback = await playlist_track_all({ id, limit, offset, cookie: userCookie, timestamp: Date.now() });
        const body = fallback.body || {};
        rawTracks = body.songs || body.tracks || [];
        rawPageCount = rawTracks.length;
        total = Math.max(
          expectedTotal,
          Number(body.total) || 0,
          Number(body.trackCount) || 0,
          Number(body.playlist && body.playlist.trackCount) || 0
        );
      }

      const tracks = mergePlaylistTrackCacheSongs([], rawTracks.map(mapSongRecord).filter(t => t.id));
      const logicalNextOffset = offset + rawPageCount;
      if (!total && metadataResolved) total = trackIds.length;
      if (playlistMeta.trackCount < total) playlistMeta.trackCount = total;
      let more = total > 0 ? logicalNextOffset < total : rawPageCount >= limit;
      let truncated = more && logicalNextOffset >= PLAYLIST_TRACK_SAFE_LIMIT;
      if (truncated) more = false;
      const complete = !more && !truncated;
      ...
      if (!cached || offset === 0) {
        ...
        cached.playlist = playlistMeta;
        cached.total = Math.max(Number(cached.total) || 0, total);
        if (trackIds.length) cached.trackIds = trackIds.slice(0, PLAYLIST_TRACK_SAFE_LIMIT);
        cached.metadataResolved = cached.metadataResolved || metadataResolved;
      }
      if (Number(cached.nextOffset) === offset) {
        cached.tracks = mergePlaylistTrackCacheSongs(cached.tracks, tracks);
        cached.nextOffset = logicalNextOffset;
        cached.complete = complete;
      }
      cached.savedAt = Date.now();
      storePlaylistTrackCacheEntry(id, cached);

      sendJSON(res, {
        playlist: playlistMeta,
        tracks,
        total: Math.max(total, tracks.length),
        nextOffset: logicalNextOffset,
        more,
        complete,
        truncated,
        cached: false,
