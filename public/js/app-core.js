...
// Mineradio split module: Core state helpers and camera controls.
...
function normalizeWallpaperParticleMode(value) {
...
function normalizeDevelopmentLockedFxState() {
  if (!fx) return;
  fx.wallpaperMode = isDevelopmentLockedFx('wallpaperMode') ? false : fx.wallpaperMode === true;
  fx.desktopLyrics = isDevelopmentLockedFx('desktopLyrics') ? false : fx.desktopLyrics === true;
  fx.wallpaperLyrics = fx.wallpaperLyrics !== false;
...
function isHiddenForBackgroundOptimization() {
  return !!(document.hidden && isBackgroundReleaseMode());
}
...
function updateDesktopRuntimeState(state) {
  ...
  if (fx && fx.desktopLyrics) setTimeout(syncDesktopOverlayState, 0);
...
function currentSectionVisualDrive() {
  // 安静段克制、推进段蓄力，确认进入副歌后再完整释放。
  var legacyDrive = lyricSunEnergy * 1.42 + lyricSunHold * 0.44;
  var buildupDrive = (adaptiveMusicDynamics.buildup || 0) * 0.72;
  var chorusDrive = (adaptiveMusicDynamics.chorus || 0) * 1.08;
  return clamp01(Math.max(legacyDrive, buildupDrive, chorusDrive));
}
...
function cameraDynamicsScale(extra) {
  ...
  // 主歌保持镜头克制，持续高能量/副歌段再逐步释放；沿用现有能量检测，不改变镜头语言。
  var sectionCameraScale = isDj ? 1 : (0.80 + currentSectionVisualDrive() * 0.48);
  return clampRange((cinemaDynamics.scale || 0.82) * (cinemaTrackProfile.scale || 1) * (extra == null ? 1 : extra) * djBoost * sectionCameraScale, isDj ? 0.24 : 0.18, isDj ? 1.42 : 1.28);
}
...
function resetAudioVisualState() {
  ...
  adaptiveMusicDynamics.rawRmsAvg = 0;
  adaptiveMusicDynamics.visualGain = 1;
  adaptiveMusicDynamics.shortEnergy = 0;
  adaptiveMusicDynamics.longEnergy = 0;
  adaptiveMusicDynamics.shortLow = 0;
  adaptiveMusicDynamics.longLow = 0;
  adaptiveMusicDynamics.buildup = 0;
  adaptiveMusicDynamics.chorus = 0;
  adaptiveMusicDynamics.chorusHold = 0;
  adaptiveMusicDynamics.chorusEntryPulse = 0;
  adaptiveMusicDynamics.frames = 0;
  if (djMode.active) resetDjModeMeter();
}

function updateAdaptiveMusicDynamics(dt, rawRms, bands) {
  var state = adaptiveMusicDynamics;
  dt = Math.max(0.001, Math.min(0.080, Number(dt) || 0.016));
  bands = bands || {};
  state.frames++;
  var rmsFollow = 1 - Math.exp(-dt / (rawRms > state.rawRmsAvg ? 2.2 : 7.5));
  state.rawRmsAvg += (rawRms - state.rawRmsAvg) * rmsFollow;
  var targetGain = clampRange(0.145 / Math.max(0.055, state.rawRmsAvg || rawRms || 0.10), 0.82, 1.34);
  if (state.frames < 45) targetGain = Math.min(targetGain, 1.12);
  state.visualGain += (targetGain - state.visualGain) * (1 - Math.exp(-dt / 2.4));

  var normalizedEnergy = clamp01((Number(bands.energy) || 0) * Math.pow(state.visualGain, 0.46));
  var normalizedLow = clamp01((Number(bands.low) || 0) * Math.pow(state.visualGain, 0.38));
  var normalizedMid = clamp01((Number(bands.mid) || 0) * Math.pow(state.visualGain, 0.34));
  var normalizedHigh = clamp01((Number(bands.high) || 0) * Math.pow(state.visualGain, 0.30));
  var normalizedVocal = clamp01((Number(bands.vocal) || 0) * Math.pow(state.visualGain, 0.34));
  var density = clamp01(normalizedEnergy * 0.38 + normalizedLow * 0.23 + normalizedMid * 0.17 + normalizedHigh * 0.10 + normalizedVocal * 0.12);

  state.shortEnergy += (density - state.shortEnergy) * (1 - Math.exp(-dt / 1.15));
  state.longEnergy += (density - state.longEnergy) * (1 - Math.exp(-dt / 9.0));
  state.shortLow += (normalizedLow - state.shortLow) * (1 - Math.exp(-dt / 1.0));
  state.longLow += (normalizedLow - state.longLow) * (1 - Math.exp(-dt / 7.2));

  var energyLift = state.shortEnergy - state.longEnergy;
  var lowLift = state.shortLow - state.longLow;
  var absoluteGate = clamp01((state.shortEnergy - 0.43) / 0.28);
  var contrastGate = clamp01((energyLift - 0.035) / 0.17);
  var lowGate = clamp01((lowLift + normalizedLow * 0.18 - 0.08) / 0.20);
  var buildupTarget = clamp01((energyLift - 0.012) / 0.14) * clamp01((state.shortEnergy - 0.30) / 0.30);
  state.buildup += (buildupTarget - state.buildup) * (1 - Math.exp(-dt / (buildupTarget > state.buildup ? 0.46 : 1.7)));
  var chorusTarget = clamp01(absoluteGate * 0.52 + contrastGate * 0.34 + lowGate * 0.24);
  if (chorusTarget > 0.54) state.chorusHold = Math.max(state.chorusHold, 1.8);
  else state.chorusHold = Math.max(0, state.chorusHold - dt);
  if (state.chorusHold > 0) chorusTarget = Math.max(chorusTarget, 0.46);
  var previousChorus = state.chorus;
  state.chorus += (chorusTarget - state.chorus) * (1 - Math.exp(-dt / (chorusTarget > state.chorus ? 0.75 : 2.8)));
  if (previousChorus < 0.50 && state.chorus >= 0.50) state.chorusEntryPulse = 1;
  else state.chorusEntryPulse *= Math.pow(0.12, dt);
  ...
  return {
    ...
    chorus: state.chorus,
    entryPulse: state.chorusEntryPulse,
    visualGain: state.visualGain
  };
}
...
function scheduleBeatCamera(beat, source) {
  ...
  // Keep rhythm as a restrained accent. Long section envelopes, color and
  // bloom carry the emotion; the camera should never jump on every beat.
  amp *= dj ? 0.52 : 0.34;
  zoomAmp *= dj ? 0.44 : 0.36;
  thetaAmp *= dj ? 0.48 : 0.42;
  phiAmp *= dj ? 0.48 : 0.42;
  rollAmp *= dj ? 0.40 : 0.32;
  if (dj && isDjMapSource && amp > 0.74) amp = 0.74 + (amp - 0.74) * 0.56;
  if (dj && isDjMapSource && zoomAmp > 0.30) zoomAmp = 0.30 + (zoomAmp - 0.30) * 0.52;
  amp = Math.max(dj ? (isDjMapSource ? 0.015 : 0.022) : 0.025, Math.min(dj ? (isDjMapSource ? 0.42 : 0.30) : 0.30, amp));
  if (isLiveSource) {
...
function updateCamera(dt) {
  ...
  var cameraShake = clampRange(Number(fx.cinemaShake) || 0, 0, 1.8);
  camera.rotation.z += beatCam.rollKick * cameraShake * 0.32;

  var cameraPunch = Math.max(camPunch * 0.55, beatCam.punch * 0.14 + beatCam.radiusKick * 0.04) * cameraShake;
  var targetFOV = BASE_FOV - cameraPunch * (djMode.active ? 0.85 : 0.75);
...
function updateCinema(dt) {
  ...
  var beatDamp = (orbit.focus.active ? (dj ? 0.66 : 0.55) : (dj ? 1.12 : 1.0)) * shake * 0.28;
  ...
  orbit.cineRadius = Math.sin(cinemaT * 0.04 + 2.0) * 0.080 * idleDamp - beatCam.radiusKick * beatDamp * (dj ? 1.22 : 1.18) - (dj ? 0 : (adaptiveMusicDynamics.chorus || 0) * 0.025 * shake);
}
