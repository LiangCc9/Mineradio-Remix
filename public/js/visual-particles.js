...
// Mineradio split module: Particle, lyric and cover visual systems.
...
function applySkullCameraPose(dt) {
  ...
  var wallpaperFull = !document.body.classList.contains('wallpaper-hosted') || normalizeWallpaperParticleMode(fx.wallpaperParticleMode) === 'full';
  var active = fx && fx.preset === SKULL_PRESET_INDEX && wallpaperFull;
...
function updateSkullParticleLayer(dt) {
  ...
  var flashTarget = clampRange(Math.pow(beatTransient, 1.34) * 0.25 + Math.max(0, bass - 0.60) * 0.08 * beatTransient, 0, 1);
  skullBeatFlash += (flashTarget - skullBeatFlash) * Math.min(1, dt * (flashTarget > skullBeatFlash ? 6.0 : 3.4));
  ...
  var ampTarget = clampRange(bass * 0.006 + mid * 0.004 + skullBeatFlash * 0.012, 0, 0.040);
...
function stageLyricTargetQuaternion(baseQuat, tiltX, tiltY, roll) {
  lyricTiltEuler.set((tiltX || 0) * Math.PI / 180, (tiltY || 0) * Math.PI / 180, roll || 0, 'YXZ');
...
// The desktop shell deliberately binds the first free port starting at 3000.
// localStorage is scoped to the complete origin (including that port), so a
// leftover preview/server process can otherwise make the wallpaper switch look
// reset on the next launch. Cookies are host-scoped rather than port-scoped;
// keep this one non-sensitive boolean there as the cross-port source of truth.
// The passive wallpaper renderer must never persist its forced `true` state.
var WALLPAPER_MODE_COOKIE_NAME = 'mineradio_wallpaper_mode_v1';

function readPersistedWallpaperMode(raw) {
  var fallback = !!(raw && raw.wallpaperMode === true);
  try {
    var prefix = WALLPAPER_MODE_COOKIE_NAME + '=';
    var cookies = String(document.cookie || '').split(';');
    for (var i = 0; i < cookies.length; i++) {
      var item = cookies[i].trim();
      if (item.indexOf(prefix) !== 0) continue;
      var value = decodeURIComponent(item.slice(prefix.length)).toLowerCase();
...
function persistWallpaperMode(enabled) {
  try {
    if (window.__MINERADIO_SURFACE__ === 'wallpaper') return;
    document.cookie = WALLPAPER_MODE_COOKIE_NAME + '=' + (enabled ? '1' : '0')
...
function readSavedLyricLayout() {
  try {
    ...
    return {
      ...
      wallpaperMode: readPersistedWallpaperMode(raw),
      ...
      wallpaperParticleMode: normalizeWallpaperParticleMode(raw.wallpaperParticleMode || fxDefaults.wallpaperParticleMode),
...
function saveLyricLayout() {
  try {
    // Persist independently before the much larger JSON write. This also keeps
    // the explicit OFF choice when localStorage is unavailable or over quota.
    persistWallpaperMode(fx.wallpaperMode === true);
    ...
    localStorage.setItem(LYRIC_LAYOUT_STORE_KEY, JSON.stringify({
      ...
      wallpaperLyrics: fx.wallpaperLyrics !== false,
      wallpaperParticleMode: normalizeWallpaperParticleMode(fx.wallpaperParticleMode),
...
function readCoverPaletteCacheStore() {
  ...
  Object.keys(store.entries).forEach(function(key){
    ...
    if (!entry || now - Number(entry.savedAt || 0) > COVER_PALETTE_CACHE_TTL_MS) delete store.entries[key];
  });
  coverPaletteCacheMemory = store;
  return store;
}

function coverPaletteCacheKey(raw) {
  return coverDepthCacheId(String(raw || '').replace(/\|tex=\d+x\d+$/, ''));
}

function applyCachedCoverPalette(raw) {
  var key = coverPaletteCacheKey(raw);
  if (!key) return false;
  var store = readCoverPaletteCacheStore();
  var entry = store.entries[key];
  if (!entry || !entry.palette || Date.now() - Number(entry.savedAt || 0) > COVER_PALETTE_CACHE_TTL_MS) {
    if (entry) delete store.entries[key];
    return false;
  }
  entry.lastUsedAt = Date.now();
  stageLyrics.coverPalette = entry.palette;
  if (fx.lyricColorMode !== 'custom') setStageLyricPalette(entry.palette);
  if (entry.accent) applyUiAccentColor(entry.accent);
  return true;
}

function persistCoverPalette(raw, palette, accent) {
  var key = coverPaletteCacheKey(raw);
  if (!key || !palette) return;
  var store = readCoverPaletteCacheStore();
  var now = Date.now();
  store.entries[key] = { palette: palette, accent: accent || '', savedAt: now, lastUsedAt: now };
  ...
  keys.slice(100).forEach(function(cacheKey){ delete store.entries[cacheKey]; });
  try { localStorage.setItem(COVER_PALETTE_CACHE_KEY, JSON.stringify(store)); }
  catch (e) {
    keys.slice(50).forEach(function(cacheKey){ delete store.entries[cacheKey]; });
    try { localStorage.setItem(COVER_PALETTE_CACHE_KEY, JSON.stringify(store)); } catch (_) {}
  }
}

function updateLyricPaletteFromCover(coverCanvas, paletteKey) {
  ...
  try {
    ...
    var accent = rgbToHexColor(accentRgb.r, accentRgb.g, accentRgb.b);
    applyUiAccentColor(accent);
    persistCoverPalette(paletteKey, stageLyrics.coverPalette, accent);
  } catch (e) {}
}
...
function buildLyricMesh(text, trans) {
  text = String(text || '').replace(/\s+/g, ' ').trim();
  ...
  if (trans && trans !== text) {
    ...
    tMesh.position.set(0, -worldH * 0.66 - tWorldH * 0.04, 0.004);
    group.add(tMesh);
...
function showStageLine(text, redrawOnly, trans) {
  ...
  stageLyrics.currentTrans = trans || '';
  var mesh = buildLyricMesh(text, trans);
...
function refreshCurrentLyricStyle() {
  ...
  showStageLine(stageLyrics.currentText, true, stageLyrics.currentTrans);
...
function updateStageLyrics3D(dt) {
  if (!stageLyrics.group) return;
  var wallpaperLyricsVisible = !document.body.classList.contains('wallpaper-hosted') || fx.wallpaperLyrics !== false;
  stageLyrics.group.visible = wallpaperLyricsVisible;
  if (!wallpaperLyricsVisible) return;
  ...
  var sectionDrive = currentSectionVisualDrive();
  var musicBloom = Math.max(sectionDrive * 0.96, beatPulse * 0.10);
  var beatGlowRaw = fx.lyricGlowBeat && lyricGlowStrength > 0
    ? Math.max(beatPulse * 0.35, beatCam.punch * 0.24 + beatCam.radiusKick * 0.45)
    : 0;
  stageLyrics.beatGlow += (beatGlowRaw - stageLyrics.beatGlow) * (beatGlowRaw > stageLyrics.beatGlow ? 0.12 : 0.075);
  if (!isFinite(stageLyrics.beatGlow)) stageLyrics.beatGlow = 0;
  var skullLyricPreset = !!(fx && fx.preset === SKULL_PRESET_INDEX);
  var solarBloom = lyricGlowStrength > 0 ? (0.18 + glowBreath * 0.16 + musicBloom * 0.90 + stageLyrics.beatGlow * 0.22 + Math.sin(t * 0.37 + 1.2) * 0.035) * glowDrive : 0;
  if (skullLyricPreset && lyricGlowStrength > 0) {
    solarBloom = (0.035 + glowBreath * 0.030 + musicBloom * 0.11 + Math.pow(Math.max(0, stageLyrics.beatGlow), 1.26) * 0.36 + Math.pow(Math.max(0, skullBeatFlash || 0), 1.08) * 0.25) * glowDrive;
  }
  ...
  var followXTarget = followDrive * (beatCam.thetaKick * 8 + beatCam.rollKick * 2);
  var followYTarget = followDrive * (beatCam.phiKick * 10 - beatCam.radiusKick * 0.12);
  var followRollTarget = followDrive * (beatCam.rollKick * 5 + beatCam.thetaKick * 2.4);
  ...
  if (skullMouthLyrics) {
    ...
    var cinemaLyricDrive = fx.cinema && !shelfDetailOpen ? clampRange(Number(fx.cinemaShake) || 0, 0, 1.6) : 0;
    var cinemaLyricX = cinemaLyricDrive * clampRange(stageLyrics.glowFollowX * 0.018 + beatCam.thetaKick * 0.34, -0.025, 0.025);
    var cinemaLyricY = cinemaLyricDrive * clampRange(stageLyrics.glowFollowY * 0.016 + beatCam.phiKick * 0.30 - beatCam.radiusKick * 0.012, -0.018, 0.018);
    var cinemaLyricRoll = cinemaLyricDrive * clampRange(stageLyrics.glowFollowRoll * 0.030 + beatCam.rollKick * 0.18, -0.008, 0.008);
    lyricCameraTarget.addScaledVector(lyricCameraRight, cinemaLyricX).addScaledVector(lyricCameraUp, cinemaLyricY);
    stageLyricTargetQuaternion(camera.quaternion, layoutTiltX, layoutTiltY, cinemaLyricRoll);
    if (stageLyrics.snapCameraLockFrames > 0) {
      stageLyrics.group.position.copy(lyricCameraTarget);
      stageLyrics.group.quaternion.copy(lyricTargetQuat);
      stageLyrics.snapCameraLockFrames -= 1;
    } else {
      var lyricFollowDt = clampRange(Number(dt) || 1 / 60, 1 / 240, 0.08);
      var lockPosRate = wallpaperLyricLock ? (wallpaperShelfLyrics ? 32.7 : 24.9) : 16.5;
      var lockQuatRate = wallpaperLyricLock ? (wallpaperShelfLyrics ? 34.8 : 26.8) : 14.9;
      var lockPosEase = 1 - Math.exp(-lockPosRate * lyricFollowDt);
      var lockQuatEase = 1 - Math.exp(-lockQuatRate * lyricFollowDt);
  ...
  function tickMesh(mesh, isCurrent) {
    ...
    if (isCurrent) {
      ...
      if (data.glowMat) {
        var glowTarget = lyricGlowStrength > 0 ? Math.min(shelfDetailLyricProfile.glowCap, (0.075 + solar * 0.34 + stageLyrics.beatGlow * 0.04 * shelfDetailLyricDim) * Math.min(3.0, glowDrive)) : 0;
      ...
      if (data.sparkMat) {
        var sparkTarget = lyricGlowStrength > 0 && fx.lyricGlowParticles && !shelfDetailOpen ? Math.min(0.52, (0.070 + sectionDrive * 0.105 + solar * 0.15 + stageLyrics.beatGlow * (0.065 + sectionDrive * 0.085)) * Math.min(1.7, glowDrive)) : 0;
        ...
        var sparkSizeTarget = fx.lyricGlowParticles && !shelfDetailOpen ? (0.045 + sectionDrive * 0.014 + solar * 0.018 + stageLyrics.beatGlow * (0.018 + sectionDrive * 0.016) + bass * 0.008) : 0.035;
      ...
      if (data.sunMat) {
        var sunTarget = lyricGlowStrength > 0 && !shelfDetailOpen ? Math.min(0.88, (Math.pow(Math.min(1.35, solar), 1.08) * 0.28 + stageLyrics.beatGlow * 0.04) * Math.min(2.4, glowDrive)) : 0;
      ...
      if (data.sun) {
        var sunPulse = solar;
        var beatScale = fx.lyricGlowBeat ? stageLyrics.beatGlow * 0.04 : 0;
      ...
      if (skullMouthLyrics) {
        ...
        mesh.scale.setScalar(0.96 + a * 0.055 + breathe + bass * 0.038 + beatPulse * 0.014 + sectionDrive * 0.035);
      ...
      if (data.sparks && data.basePositions) {
        ...
        for (var si = 0; si < arr.length / 3; si++) {
          ...
          var sectionParticle = 0.68 + sectionDrive * 0.90;
          var dustBreath = fx.lyricGlowParticles ? (0.62 + 0.38 * Math.sin(t * (0.32 + (si % 7) * 0.025) + s)) : 0.18;
          var drift = fx.lyricGlowParticles ? 1 : 0.30;
          arr[si*3] = base[si*3] + Math.sin(t * (0.18 + (si % 5) * 0.025) + s) * (0.045 + bass * 0.030 + particleBeat * 0.012) * drift * sectionParticle + Math.cos(t * 0.11 + s) * 0.018 * dustBreath;
          arr[si*3+1] = base[si*3+1] + Math.cos(t * (0.16 + (si % 6) * 0.024) + s) * (0.042 + mid * 0.026 + particleBeat * 0.010) * drift * sectionParticle + Math.sin(t * 0.13 + s) * 0.016 * dustBreath;
          arr[si*3+2] = base[si*3+2] + Math.sin(t * (0.24 + (si % 4) * 0.035) + s) * (0.036 + particleBeat * 0.008) * drift * sectionParticle;
        }
...
function tickLyricsParticles() {
  ...
  if (newIdx !== stageLyrics.currentIdx) {
    ...
    showStageLine(lyricsLines[newIdx].text || '', false, lyricsLines[newIdx].trans || '');
  }
  if (stageLyrics.current) {
...
function startColorMixTween(durationMs) {
  ...
  if (uniforms.uCoverMorph) uniforms.uCoverMorph.value = 0;
  function step(now) {
    var rawT = Math.min(1, (now - start) / durationMs);
    var t = visualEase(rawT);
    uniforms.uColorMixT.value = t;
    if (uniforms.uCoverMorph) uniforms.uCoverMorph.value = Math.sin(rawT * Math.PI);
    if (rawT < 1) colorMixTween = { raf: requestAnimationFrame(step) };
    else {
      if (uniforms.uCoverMorph) uniforms.uCoverMorph.value = 0;
      colorMixTween = null;
    }
...
function applyCoverCanvas(cv, thumbSrc, opts) {
  ...
  var paletteSeed = opts.coverKey || thumbSrc || '';
  var cacheSeed = paletteSeed + '|tex=' + (cv.width || 0) + 'x' + (cv.height || 0);
  var cachedPaletteApplied = applyCachedCoverPalette(paletteSeed);
  var cachedDepth = getCoverDepthCache(cacheSeed);
  var hadPreviousCover = uniforms.uHasCover.value > 0.5 && !!coverTex.image;
  ...
  if (hadPreviousCover && !opts.fromResolutionChange) {
    startColorMixTween(colorMixMs);
  } else {
  ...
  function refreshCoverDependentColors() {
    ...
    if (!cachedPaletteApplied) updateLyricPaletteFromCover(cv, paletteSeed);
  }
