...
// Mineradio split module: Visual settings and presets.
...
function applyUiAccentColor(coverColor) {
  var color = normalizeHexColor(coverColor || fx.uiAccentColor || '#00f5d4', '#00f5d4');
  var rgb = hexToRgb(color);
  var root = document.documentElement;
...
function updateDevelopmentFxControls() {
  [
    ...
    ['wallpaperMode', 't-wallpaperMode', '把完整 Mineradio 主界面作为 Windows 动态桌面壁纸']
...
function updateWallpaperControls() {
  var mode = normalizeWallpaperParticleMode(fx.wallpaperParticleMode);
  fx.wallpaperParticleMode = mode;
  // Wallpaper lyrics share the same render surface as the cover particles.
  // They are not a second desktop overlay and therefore remain integrated.
  fx.wallpaperLyrics = true;
  document.body.setAttribute('data-wallpaper-particles', mode);
  document.body.classList.remove('wallpaper-lyrics-hidden');
  document.documentElement.style.setProperty('--wallpaper-visual-opacity', clampRange(fx.wallpaperOpacity == null ? 1 : Number(fx.wallpaperOpacity), 0.35, 1).toFixed(3));
  document.querySelectorAll('#wallpaper-particle-seg [data-wallpaper-particles]').forEach(function(btn){
    btn.classList.toggle('active', btn.getAttribute('data-wallpaper-particles') === mode);
  });
  var lyricsToggle = document.getElementById('t-wallpaperLyrics');
  if (lyricsToggle) lyricsToggle.classList.add('on');
}

function setWallpaperParticleMode(mode, silent) {
  mode = normalizeWallpaperParticleMode(mode);
  fx.wallpaperParticleMode = mode;
  fx.wallpaperLyrics = true;
  if (mode !== 'off' && !fx.wallpaperMode) fx.wallpaperMode = true;
  updateWallpaperControls();
  var modeToggle = document.getElementById('t-wallpaperMode');
  if (modeToggle) modeToggle.classList.toggle('on', !!fx.wallpaperMode);
  saveLyricLayout();
  applyWallpaperModeState(true);
  if (!silent) {
    var label = mode === 'full' ? '封面与环境粒子' : (mode === 'ambient' ? '环境粒子' : '粒子已关闭');
    showToast('桌面壁纸: ' + label);
  }
}
...
function setPerformanceBackgroundMode(mode, silent) {
  ...
  if (!silent) {
    showToast(next === 'keep' ? '后台策略: 保持运行' : (next === 'release' ? '后台策略: 停止并释放' : '后台策略: 正常运行'));
  }
}
...
function updateFxInputs() {
  ...
  setTrackCrossfadeMs(trackCrossfadeMs, true);
  updateLyricGlowControls();
  // 同步开关
...
function relabelFxPanelControls() {
  ...
  setFxSectionBefore('wallpaper-particle-seg', '桌面粒子');
  ...
  setFxSliderLabel('fx-wallpaperopacity', '融合强度');
...
function bindFxPanel() {
  ...
  ids.forEach(function(pair){
    ...
    el.addEventListener('input', function(){
      ...
      if (pair[1] === 'wallpaperOpacity') updateWallpaperControls();
      saveLyricLayout();
    });
  });
  document.querySelectorAll('#wallpaper-particle-seg [data-wallpaper-particles]').forEach(function(btn){
    btn.addEventListener('click', function(){
      setWallpaperParticleMode(btn.getAttribute('data-wallpaper-particles'));
    });
  });
  var crossfadeSlider = document.getElementById('fx-crossfade');
  if (crossfadeSlider) {
    crossfadeSlider.addEventListener('input', function(){ setTrackCrossfadeMs(crossfadeSlider.value, true); });
    crossfadeSlider.addEventListener('change', function(){ setTrackCrossfadeMs(crossfadeSlider.value, false); });
  }
...
function toggleFx(key) {
  if (key === 'wallpaperLyrics' && !isDevelopmentLockedFx(key)) {
    fx.wallpaperLyrics = true;
    updateWallpaperControls();
    saveLyricLayout();
    if (fx.wallpaperMode) applyWallpaperModeState(true);
    showToast('壁纸歌词已与动态壁纸合并');
    return;
  }
  ...
  fx[key] = !fx[key];
  if (key === 'wallpaperMode') fx.wallpaperLyrics = true;
  var toggleId = 't-' + (key === 'floatLayer' ? 'float' : key === 'aiDepth' ? 'aidepth' : key);
  var toggle = document.getElementById(toggleId);
  if (toggle) toggle.classList.toggle('on', fx[key]);
  syncFxUniforms();
  if (key === 'lyricCameraLock' || key === 'lyricGlow' || key === 'lyricGlowBeat' || key === 'lyricGlowParticles' || key === 'bloom' || key === 'edge' || key === 'cinema' || key === 'desktopLyrics' || key === 'desktopLyricsClickThrough' || key === 'desktopLyricsCinema' || key === 'desktopLyricsHighlight' || key === 'wallpaperMode' || key === 'wallpaperLyrics' || key === 'shelfShowPodcasts' || key === 'shelfMergeCollections' || key === 'liveBackgroundKeep') saveLyricLayout();
  ...
  if (key === 'wallpaperMode' || key === 'wallpaperLyrics') {
    updateWallpaperControls();
    var wallpaperModeToggle = document.getElementById('t-wallpaperMode');
    if (wallpaperModeToggle) wallpaperModeToggle.classList.toggle('on', !!fx.wallpaperMode);
    applyWallpaperModeState(true);
  ...
  if (key === 'wallpaperMode') showToast(fx.wallpaperMode ? '桌面壁纸已开启 · 播放控制仍在 App' : '桌面壁纸已关闭');
  if (key === 'wallpaperLyrics') showToast(fx.wallpaperLyrics ? '壁纸歌词已开启' : '壁纸歌词已隐藏');
...
function setImmersiveMode(on) {
  on = !!on;
  if (on && !(currentCoverSong && currentCoverSong()) && !(audio && audio.src)) {
    homeForcedOpen = true;
    homeSuppressed = false;
    updateEmptyHomeVisibility({ forceLoad: false });
    showToast('先播放一首歌再进入沉浸模式');
    return;
  }
