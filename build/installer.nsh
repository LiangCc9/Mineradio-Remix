...
!macro customInstall
  ...
  ${IfNot} ${Errors}
    FileWrite $0 "Mineradio Remix install root$\r$\n"
    FileWrite $0 "appId=com.liangcc9.mineradio.remix$\r$\n"
    FileClose $0
  ${EndIf}
!macroend
...
Function MineradioUseFirstAvailableInstallDir
  ...
  StrCpy $INSTDIR "C:\MineradioRemix"
  Return

  driveD:
    StrCpy $INSTDIR "D:\MineradioRemix"
    Return
  driveE:
    StrCpy $INSTDIR "E:\MineradioRemix"
    Return
  driveF:
    StrCpy $INSTDIR "F:\MineradioRemix"
    Return
  driveG:
    StrCpy $INSTDIR "G:\MineradioRemix"
    Return
  driveH:
    StrCpy $INSTDIR "H:\MineradioRemix"
    Return
  driveI:
    StrCpy $INSTDIR "I:\MineradioRemix"
    Return
  driveJ:
    StrCpy $INSTDIR "J:\MineradioRemix"
    Return
  driveK:
    StrCpy $INSTDIR "K:\MineradioRemix"
    Return
  driveL:
    StrCpy $INSTDIR "L:\MineradioRemix"
    Return
  driveM:
    StrCpy $INSTDIR "M:\MineradioRemix"
    Return
  driveN:
    StrCpy $INSTDIR "N:\MineradioRemix"
    Return
  driveO:
    StrCpy $INSTDIR "O:\MineradioRemix"
    Return
  driveP:
    StrCpy $INSTDIR "P:\MineradioRemix"
    Return
  driveQ:
    StrCpy $INSTDIR "Q:\MineradioRemix"
    Return
  driveR:
    StrCpy $INSTDIR "R:\MineradioRemix"
    Return
  driveS:
    StrCpy $INSTDIR "S:\MineradioRemix"
    Return
  driveT:
    StrCpy $INSTDIR "T:\MineradioRemix"
    Return
  driveU:
    StrCpy $INSTDIR "U:\MineradioRemix"
    Return
  driveV:
    StrCpy $INSTDIR "V:\MineradioRemix"
    Return
  driveW:
    StrCpy $INSTDIR "W:\MineradioRemix"
    Return
  driveX:
    StrCpy $INSTDIR "X:\MineradioRemix"
    Return
  driveY:
    StrCpy $INSTDIR "Y:\MineradioRemix"
    Return
  driveZ:
    StrCpy $INSTDIR "Z:\MineradioRemix"
    Return
FunctionEnd
...
Function MineradioNormalizeInstallDir
  ...
  StrCpy $2 "$0" 15 -15
  ${If} $1 < 15
  ${OrIf} $2 != "\MineradioRemix"
  ${AndIf} $2 != "\mineradioremix"
    StrCpy $0 "$0\MineradioRemix"
  ${EndIf}
  Exch $0
FunctionEnd
...
Function MineradioDisableUnsafeOldUninstallers
  ...
  Pop $1
  ${If} $1 == "1"
    DetailPrint "Skip unsafe legacy Mineradio Remix uninstaller: $0"
    StrCpy $2 "1"
  ${EndIf}
...
Function MineradioDeleteLegacyUninstallerFileIfMissingMarker
  ...
  ${If} $0 != ""
    ...
    ${If} $1 != ""
      ...
      DetailPrint "Remove legacy Mineradio Remix uninstaller file: $1"
...
Function MineradioValidateInstallDir
  ...
  ${If} $1 == ":"
    ...
    ${OrIf} $0 == "c"
      ...
      ${AndIf} $3 != "1"
      ${AndIf} $4 != "1"
        MessageBox MB_ICONSTOP|MB_OK "检测到这台电脑还有 D-Z 盘，Mineradio Remix 不安装到 C 盘。请改选 D 盘或其它非 C 盘的 Mineradio Remix 文件夹。$\r$\n$\r$\n如果电脑只有 C 盘，安装器会自动放行 C:\MineradioRemix。"
        Abort
      ${EndIf}
    ${EndIf}
  ${EndIf}
  ...
  ${If} $0 < 15
  ${OrIf} $1 != "\MineradioRemix"
  ${AndIf} $1 != "\mineradioremix"
    MessageBox MB_ICONSTOP|MB_OK "安装目录必须是独立的 Mineradio Remix 文件夹。请选择一个上级目录，安装器会自动创建 Mineradio Remix 子文件夹。"
    Abort
  ${EndIf}
  ...
  MessageBox MB_ICONSTOP|MB_OK "为避免卸载时误删其它文件，Mineradio Remix 不能安装到已有文件的非专属目录。请新建或选择一个空的 Mineradio Remix 文件夹。$\r$\n$\r$\n当前路径：$INSTDIR"
  Abort
...
Function MineradioWelcomeShow
  ...
  ${NSD_CreateLabel} 22u 20u 82u 10u "MINERADIO REMIX"
  Pop $0
  SendMessage $0 ${WM_SETFONT} $MineradioSmallFont 1
  SetCtlColors $0 "3257F7" "FFFFFF"

  ${NSD_CreateLabel} 22u 42u 226u 30u "Mineradio Remix 安装"
  Pop $0
  ...
  ${NSD_CreateLabel} 22u 96u 238u 24u "为这台电脑安装 Mineradio Remix。默认安装到 D:\MineradioRemix，下一步可以自由选择其它位置。"
  Pop $0
...
Function MineradioDirectoryBrowse
  nsDialogs::SelectFolderDialog "选择 Mineradio Remix 安装文件夹" "$INSTDIR"
  Pop $0
  ${If} $0 != error
  ${AndIf} $0 != ""
    Push "$0"
...
Function MineradioDirectoryShow
  ...
  ${NSD_CreateLabel} 22u 122u 238u 12u "默认推荐：D:\MineradioRemix；选盘符会自动建文件夹。"
  Pop $0
  SendMessage $0 ${WM_SETFONT} $MineradioSmallFont 1
...
Function un.MineradioValidateUninstallDir
  ...
  Pop $1
  ${If} $0 != $1
    MessageBox MB_OK|MB_ICONSTOP "当前卸载路径不是 Mineradio Remix 专属目录，已阻止卸载以避免误删其它文件。$\r$\n$\r$\n当前路径：$INSTDIR$\r$\n安全路径应为：$0"
    SetErrorLevel 2
    Quit
  ${EndIf}
  StrCpy $INSTDIR "$0"
  ...
  Pop $0
  ${If} $0 != "1"
    MessageBox MB_OK|MB_ICONSTOP "无法确认当前目录属于 Mineradio Remix，已阻止卸载以避免误删其它文件。$\r$\n$\r$\n当前路径：$INSTDIR"
    SetErrorLevel 2
    Quit
  ${EndIf}
FunctionEnd
